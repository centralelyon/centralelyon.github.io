...
var sketch = function( p ) { 
  ...
  var title_text = "Glacis Prediction";
  ...
  var update_temperature_text = function() {
    var the_color= "rgba(0,0,0,0.75)"//"rgba("+Math.round(255*temperature)+",0,"+255+",1)";
    text_temperature.style("color", the_color); // ff990a
    text_temperature.html("temperature: "+Math.round(temperature*100));
  };
  ...
  var init = function() {
    ...
    reset_button.attribute("id", "btn-clear")
    reset_button.position(10, screen_height-64-16);
    ...
    model_sel.position(146, screen_height-64-16);
    for (var i=0;i<class_list.length;i++) {
      model_sel.option(class_list[i]);
    }
    model_sel.changed(model_sel_event);

    text_model = p.createP("model");
    text_model.style("font-family", "Montserrat");
    text_model.style("font-size", "12");
    text_model.style("color", "rgba(0,0,0,0.5)")
    text_model.position(146+16, screen_height-64-16-32);

    // scale factor selection
    scale_sel = p.createInput(100)
    scale_sel.attribute("type", "number")
    scale_sel.attribute("step", "50")
    scale_sel.attribute("min", "50")
    scale_sel.attribute("max", "400")
    // scale_sel = p.createSelect();
    scale_sel.position(280, screen_height-64-16);
    // for (var i=0;i<scale_list.length;i++) {
    //   scale_sel.option(scale_list[i]);
    // }
    scale_sel.changed(model_sel_event);

    text_scale = p.createP("scale factor");
    text_scale.style("font-family", "Montserrat");
    text_scale.style("font-size", "12");
    text_scale.style("color", "rgba(0,0,0,0.5)")
    text_scale.position(280+16, screen_height-64-16-32);


    // temp
    temperature_slider = p.createSlider(1, 100, temperature*100);
    temperature_slider.position(0*screen_width/2-10*0+10, screen_height-27-16);
    ...
    // text_title.style("font-family", "Montserrat");
    // text_title.style("font-size", "20");
    // text_title.style("color", "rgba(0,0,0,0.75)"); // ff990a
    // set_title_text(title_text);

    // temperature text
    text_temperature = p.createP();
    text_temperature.style("font-family", "Montserrat");
    text_temperature.style("font-size", "14");
    text_temperature.position(screen_width-140, screen_height-64-16);
  ...
  var restart = function() {
    ...
    line_color = p.color(0,0,0); //p.color(p.random(64, 224), p.random(64, 224), p.random(64, 224));
    predict_line_color = p.color(191, 191, 191); //p.color(p.random(64, 224), p.random(64, 224), p.random(64, 224));
  ...
  //   ModelImporter.change_model(model, c, model_mode, call_back, true);
  // };

  var model_sel_event = function() {
    var c = model_sel.value();
    let sf = scale_sel.value();
    var model_mode = "gen";
    console.log("user wants to change model "+c+" with scale factor "+sf);
    var call_back = function(new_model) {
      model = new_model;
      model.set_pixel_factor(screen_scale_factor);
      encode_strokes(strokes);
    ...
    // set_title_text('loading '+c+' model...');
    ModelImporter.change_model(model, c, model_mode, call_back, true, sf);
  };
