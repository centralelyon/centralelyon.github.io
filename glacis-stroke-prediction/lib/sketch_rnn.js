...
(function(global) {
  ...
  /**
   * assume given a 
   * model model_raw_data, put it inside the model archives.
   ...
   */
  var change_model = function(model, class_name, class_type, call_back, do_not_cache, fs=100) {
    if (model && typeof(class_name) === "string" && typeof(class_type) === "string") {
      ...
      if (model_data_archive[model_name]) {
        ...
        loadJSON("models/"+model_name+'.json', function(response) {
        // loadJSON(model_url+model_name+'.json', function(response) {
          console.log("callback from json load");
          // Parse JSON string into object
          var result = JSON.parse(response);
          result[0].scale_factor = parseInt(fs) //result[0].scale_factor * fs_mult;
...
/**
 * Internal Layer Normalization LSTM class used by sketch-rnn
 * @class
 * @constructor
 */
function LayerNormLSTMCell(num_units, input_size, Wxh, Whh, ln_all_gamma, ln_all_beta, ln_c_gamma, ln_c_beta) {
  this.num_units = num_units;
  this.input_size = input_size;
  this.forget_bias = 1.0;
  this.Wxh = Wxh;
  ...
  this.ln_all_gamma = ln_all_gamma;
  this.ln_all_beta = ln_all_beta;
  this.ln_c_gamma = ln_c_gamma;
  this.ln_c_beta = ln_c_beta;
}

LayerNormLSTMCell.prototype.zero_state = function() {
  return [nj.zeros(this.num_units), nj.zeros(this.num_units)];
};

LayerNormLSTMCell.prototype.forward = function(x, h, c) {
  var concat = nj.concatenate([x, h]);
  var hidden = nj.dot(concat, this.Wfull); // No bias?
  var num_units = this.num_units;

  // Layer Norm All
  console.log("hidden: "+hidden);
  console.log("this.ln_all_gamma: "+this.ln_all_gamma);
  console.log("this.ln_all_beta: "+this.ln_all_beta);
  //hidden = nj.add(nj.multiply(hidden,this.ln_all_gamma),this.ln_all_beta);
  hidden = layerNormalization(hidden, this.ln_all_gamma, this.ln_all_beta);
  var i=hidden.slice([0*num_units, 1*num_units]);
  var j=hidden.slice([1*num_units, 2*num_units]);
  var f=hidden.slice([2*num_units, 3*num_units]);
  var o=hidden.slice([3*num_units, 4*num_units]);

  var new_c = nj.add(nj.multiply(c,nj.sigmoid(nj.add(f,this.forget_bias))),nj.multiply(nj.sigmoid(i),nj.tanh(j)));
  var new_h = nj.multiply(nj.tanh(layerNormalization(new_c,this.ln_c_gamma,this.ln_c_beta)),nj.sigmoid(o))

  return [new_h, new_c];
};

LayerNormLSTMCell.prototype.encode = function(sequence) {
  var x;
  var state = this.zero_state();
  var h = state[0];
  var c = state[1];
...
function layerNormalization(x, gamma, beta) {
  var epsilon = 1e-3;
  var mean = nj.mean(x);
  var std = nj.std(x);
  var stdeps = nj.array(Array(x.shape[0]).fill(std+epsilon))
  console.log("stdeps: "+stdeps.shape)
  console.log("mean: "+mean.shape)
  console.log("x-mean: "+nj.subtract(x, mean).shape)
  var norm = nj.divide(nj.subtract(x, mean), stdeps);
  return nj.add(nj.multiply(gamma, norm), beta);
}
...
function SketchRNN(model_raw_data) {
  ...
  var dec_lstm_ln_gamma, dec_lstm_ln_beta, dec_lstm_ln_c_gamma, dec_lstm_ln_c_beta; 
  // Layer Normalization Encoder
  var enc_fw_lstm_ln_all_gamma,enc_fw_lstm_ln_all_beta,enc_fw_lstm_ln_c_gamma,enc_fw_lstm_ln_c_beta,enc_bw_lstm_ln_all_gamma,enc_bw_lstm_ln_all_beta,enc_bw_lstm_ln_c_gamma,enc_bw_lstm_ln_c_beta
  ...
  function load_model(model_raw_data) {
    ...
    console.log(model_raw_data) // DEBUG
    ...
    for (i=0;i<num_blobs;i++) {
      if (info.compressed === 0 ){
        weights[i] = nj.array(new Float32Array(weightsIn[i]), 'float32'); 
      }
      else{
    ...
    if(info.mode === 2 || info.mode === "gen") { // 0 or 1 - vae, 2 - gen
      if (weights.length === 5){
        ...
        dec_lstm_W_xh = weights[2];
        dec_lstm_W_hh = weights[3];
        dec_lstm_ln_gamma = weights[4];
        dec_lstm_ln_beta = weights[5];
        dec_lstm_ln_c_gamma = weights[6];
        dec_lstm_ln_c_beta = weights[7];


        dec_num_units = dec_lstm_W_hh.shape[0];
        dec_input_size = dec_lstm_W_xh.shape[0];
    
        dec_lstm = new LayerNormLSTMCell(dec_num_units, dec_input_size, dec_lstm_W_xh, dec_lstm_W_hh, dec_lstm_ln_gamma, dec_lstm_ln_beta, dec_lstm_ln_c_gamma, dec_lstm_ln_c_beta);
      }
    } 
    else { // VAE
      if (weights.length === 17){ // Regular LSTM
      ...
      else if (weights.length === 26){ // Layer Normalization LSTM
        enc_fw_lstm_W_xh = weights[0];
        enc_fw_lstm_W_hh = weights[1];
        enc_fw_lstm_ln_all_gamma = weights[2];
        enc_fw_lstm_ln_all_beta = weights[3];
        enc_fw_lstm_ln_c_gamma = weights[4];
        enc_fw_lstm_ln_c_beta = weights[5];
  
        enc_bw_lstm_W_xh = weights[6];
        enc_bw_lstm_W_hh = weights[7];
        enc_bw_lstm_ln_all_gamma = weights[8];
        enc_bw_lstm_ln_all_beta = weights[9];
        enc_bw_lstm_ln_c_gamma = weights[10];
        enc_bw_lstm_ln_c_beta = weights[11];

        enc_mu_w = weights[12];
        enc_mu_b = weights[13];
        enc_sigma_w = weights[14];
        enc_sigma_b = weights[15];
        enc_w = weights[16];
        enc_b = weights[17];

        dec_output_w = weights[18];
        dec_output_b = weights[19];
        dec_lstm_W_xh = weights[20];
        dec_lstm_W_hh = weights[21];
        dec_lstm_ln_gamma = weights[22];
        dec_lstm_ln_beta = weights[23];
        dec_lstm_ln_c_gamma = weights[24];
        dec_lstm_ln_c_beta = weights[25];

        enc_num_units = enc_fw_lstm_W_hh.shape[0];
        enc_input_size = enc_fw_lstm_W_xh.shape[0];
        z_size = enc_w.shape[0];
  
        enc_fw_lstm = new LayerNormLSTMCell(enc_num_units, enc_input_size, enc_fw_lstm_W_xh, enc_fw_lstm_W_hh, enc_fw_lstm_ln_all_gamma, enc_fw_lstm_ln_all_beta, enc_fw_lstm_ln_c_gamma, enc_fw_lstm_ln_c_beta);
        enc_bw_lstm = new LayerNormLSTMCell(enc_num_units, enc_input_size, enc_bw_lstm_W_xh, enc_bw_lstm_W_hh, enc_bw_lstm_ln_all_gamma, enc_bw_lstm_ln_all_beta, enc_bw_lstm_ln_c_gamma, enc_bw_lstm_ln_c_beta);
        dec_num_units = dec_lstm_W_hh.shape[0];
        dec_input_size = dec_lstm_W_xh.shape[0];
